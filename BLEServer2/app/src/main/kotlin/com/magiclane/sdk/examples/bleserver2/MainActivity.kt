...
class MainActivity: AppCompatActivity()
{
    ...
    private fun createNavigationService(): BluetoothGattService
    {
        ...
        val turnInstruction = BluetoothGattCharacteristic(
            TURN_INSTRUCTION, // Read-only characteristic, supports notifications
    ...
    private fun notifyRegisteredDevices(data: ByteArray)
    {
            ...
            (ActivityCompat.checkSelfPermission(this, Manifest.permission.BLUETOOTH_CONNECT) == PackageManager.PERMISSION_GRANTED))
        {
            if (registeredDevices.isNotEmpty())
            {
                ...
                {
                    bluetoothGattServer?.getService(NAVIGATION_SERVICE)?.getCharacteristic(TURN_INSTRUCTION)?.let {
                        @Suppress("DEPRECATION")
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU)
                        {
    ...
    {
        val turnDistance = (navInstructionDistance.text ?: " ").toString()
