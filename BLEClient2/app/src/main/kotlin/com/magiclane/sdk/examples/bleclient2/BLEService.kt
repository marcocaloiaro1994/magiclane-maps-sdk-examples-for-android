...
import com.magiclane.sdk.examples.bleclient2.SampleGattAttributes.CLIENT_CONFIG
import com.magiclane.sdk.examples.bleclient2.SampleGattAttributes.TURN_INSTRUCTION
...
class BLEService : Service() {
    ...
    private var turnImageData = byteArrayOf(0, 0, 0, 0)

    enum class TDataType(val value: Int) {
        ETurnImage(0),
        ETurnDistance(1),
        ETurnInstruction(2),
        ERemainingTravelTime(3),
        ERemainingTravelDistance(4),
        ESpeed(5),
    }
        ...
        override fun onServiceChanged(gatt: BluetoothGatt) {
            if ((Build.VERSION.SDK_INT < Build.VERSION_CODES.S) ||
                ...
                bluetoothGatt?.let {
                    ...
                    ).postDelayed({ connect(bluetoothDeviceAddress) }, 1000)
                }
            }
        }
    }
        ...
        if ((data != null) && data.isNotEmpty()) {
            if (TURN_INSTRUCTION == characteristic.uuid) {
                if (turnInstructionSize == 0) {
                    when (data[0].toInt()) {
                        TDataType.ETurnImage.value -> // turn image
                            {
                                if (data.size == 5) {
                        ...
                        TDataType.ETurnDistance.value -> // turn distance
                            {
                                if (data.size > 1) {
                                    val byteArray = ByteArray(data.size - 1)
                                    System.arraycopy(data, 1, byteArray, 0, byteArray.size)
                                    intent.putExtra(EXTRA_TYPE, TDataType.ETurnDistance.value)
                                    intent.putExtra(EXTRA_DATA, String(byteArray))
                                }
                            }

                        TDataType.ETurnInstruction.value -> // turn instruction
                            {
                                if (data.size > 1) {
                        ...
                        TDataType.ERemainingTravelTime.value -> // remaining travel time
                            {
                                if (data.size > 1) {
                                    val byteArray = ByteArray(data.size - 1)
                                    System.arraycopy(data, 1, byteArray, 0, byteArray.size)
                                    intent.putExtra(
                                        EXTRA_TYPE,
                                        TDataType.ERemainingTravelTime.value,
                                    )
                                    intent.putExtra(EXTRA_DATA, String(byteArray))
                                }
                            }

                        TDataType.ERemainingTravelDistance.value -> // remaining travel distance
                            {
                                if (data.size > 1) {
                                    val byteArray = ByteArray(data.size - 1)
                                    System.arraycopy(data, 1, byteArray, 0, byteArray.size)
                                    intent.putExtra(
                                        EXTRA_TYPE,
                                        TDataType.ERemainingTravelDistance.value,
                                    )
                                    intent.putExtra(EXTRA_DATA, String(byteArray))
                                }
                            }

                        TDataType.ESpeed.value -> // speed
                            {
                                if (data.size > 1) {
                                    val byteArray = ByteArray(data.size - 1)
                                    System.arraycopy(data, 1, byteArray, 0, byteArray.size)
                                    intent.putExtra(EXTRA_TYPE, TDataType.ESpeed.value)
                                    intent.putExtra(EXTRA_DATA, String(byteArray))
                                }
                            }
                    }
                    ...
                    if ((turnInstructionDataOffset + data.size) <= turnInstructionData.size) {
                        ...
                        if (turnInstructionDataOffset == turnInstructionData.size) {
                            ...
                            intent.putExtra(EXTRA_TYPE, TDataType.ETurnInstruction.value)
    ...
    companion object {
        ...
        const val ACTION_GATT_CONNECTED = "com.magiclane.sdk.examples.bleclient2.ACTION_GATT_CONNECTED"
        const val ACTION_GATT_DISCONNECTED = "com.magiclane.sdk.examples.bleclient2.ACTION_GATT_DISCONNECTED"
        const val ACTION_GATT_SERVICES_DISCOVERED =
            "com.magiclane.sdk.examples.bleclient2.ACTION_GATT_SERVICES_DISCOVERED"
        const val ACTION_DATA_AVAILABLE = "com.magiclane.sdk.examples.bleclient2.ACTION_DATA_AVAILABLE"
        const val EXTRA_DATA = "com.magiclane.sdk.examples.bleclient2.EXTRA_DATA"
        const val EXTRA_TYPE = "com.magiclane.sdk.examples.bleclient2.EXTRA_TYPE"
    }
}
